...
namespace SparkyStudios::Audio::Amplitude
{
    ...
    AmUInt64 MP3Codec::MP3Decoder::Stream(AudioBuffer* out, AmUInt64 bufferOffset, AmUInt64 seekOffset, AmUInt64 length)
    {
        ...
        const AmUInt32 scratchSize = static_cast<AmUInt32>(length * _mp3.channels);
        ...
        const AmUInt64 read = drmp3_read_pcm_frames_f32(&_mp3, length, _scratch.GetBuffer());

        Deinterleave(_scratch.GetBuffer(), 0, out->GetData().GetBuffer(), bufferOffset, length, _mp3.channels);
    ...
    {
        return ampoolshared(eMemoryPoolKind_Codec, MP3Decoder, this);
    }

    std::shared_ptr<Codec::Encoder> MP3Codec::CreateEncoder()
    {
        return ampoolshared(eMemoryPoolKind_Codec, MP3Encoder, this);
    }
