...
{
    HardClipNodeInstance::HardClipNodeInstance()
        : ProcessorNodeInstance(false)
    {}

    const AudioBuffer* HardClipNodeInstance::Process(const AudioBuffer* input)
    {
        ...
        for (AmSize c = 0; c < input->GetChannelCount(); c++)
        {
            ...
            for (AmSize i = 0, l = input->GetFrameCount(); i < l; i++)
            {
                ...
                y = AM_CLAMP(x, AM_AUDIO_SAMPLE_MIN, AM_AUDIO_SAMPLE_MAX);
            }
        }
