...
{
    AM_TEST_CASE(EngineTestCase, core_faders, ease_in_out)
    {
    public:
        void Run() override
        {
            ...
            AM_EXPECT(instance->GetFromPercentage(0.25) - 0.12916193876118984 < kEpsilon);
            AM_EXPECT(instance->GetFromPercentage(0.50) - 0.50000001926874293 < kEpsilon);
            AM_EXPECT(instance->GetFromPercentage(0.75) - 0.87083807775645594 < kEpsilon);
            AM_EXPECT(instance->GetFromPercentage(1.00) == 1.0);
        }
    };

    AM_REGISTER_TEST(core_faders, ease_in_out);
} // namespace SparkyStudios::Audio::Amplitude::Tests
