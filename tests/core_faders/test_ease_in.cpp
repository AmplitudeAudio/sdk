...
{
    AM_TEST_CASE(EngineTestCase, core_faders, ease_in)
    {
    public:
        void Run() override
        {
            ...
            AM_EXPECT(instance->GetFromPercentage(0.25) - 0.09346465401576336 < kEpsilon);
            AM_EXPECT(instance->GetFromPercentage(0.50) - 0.31535681876384836 < kEpsilon);
            AM_EXPECT(instance->GetFromPercentage(0.75) - 0.62186187464895193 < kEpsilon);
            AM_EXPECT(instance->GetFromPercentage(1.00) == 1.0);
        }
    };

    AM_REGISTER_TEST(core_faders, ease_in);
} // namespace SparkyStudios::Audio::Amplitude::Tests
