...
{
    AM_TEST_CASE(EngineTestCase, core_faders, scurve_sharp)
    {
    public:
        void Run() override
        {
            ...
            AM_EXPECT(instance->GetFromPercentage(0.25) - 0.03729032857535650 < kEpsilon);
            AM_EXPECT(instance->GetFromPercentage(0.50) - 0.50000008381902017 < kEpsilon);
            AM_EXPECT(instance->GetFromPercentage(0.75) - 0.96270967398599439 < kEpsilon);
            AM_EXPECT(instance->GetFromPercentage(1.00) == 1.0);
        }
    };

    AM_REGISTER_TEST(core_faders, scurve_sharp);
} // namespace SparkyStudios::Audio::Amplitude::Tests
