...
{
    AM_TEST_CASE(EngineTestCase, core_faders, linear)
    {
    public:
        void Run() override
        {
            ...
            AM_EXPECT(instance->GetFromPercentage(0.25) == 0.25);
            AM_EXPECT(instance->GetFromPercentage(0.50) == 0.50);
            AM_EXPECT(instance->GetFromPercentage(0.75) == 0.75);
            AM_EXPECT(instance->GetFromPercentage(1.00) == 1.0);
        }
    };

    AM_REGISTER_TEST(core_faders, linear);
} // namespace SparkyStudios::Audio::Amplitude::Tests
