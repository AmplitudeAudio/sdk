...
{
    AM_TEST_CASE(EngineTestCase, core_faders, ease)
    {
    public:
        void Run() override
        {
            ...
            AM_EXPECT(instance->GetFromPercentage(0.25) - 0.40851059199373591 < kEpsilon);
            AM_EXPECT(instance->GetFromPercentage(0.50) - 0.80240338786711973 < kEpsilon);
            AM_EXPECT(instance->GetFromPercentage(0.75) - 0.96045897841111938 < kEpsilon);
            AM_EXPECT(instance->GetFromPercentage(1.00) == 1.0);
        }
    };

    AM_REGISTER_TEST(core_faders, ease);
} // namespace SparkyStudios::Audio::Amplitude::Tests
