...
#else
    // If memory stats are disabled, leak detection should still work minimally
    // or the function should exist but return a message indicating stats are disabled
    AmString leakReport = amMemory->InspectMemoryLeaks();
    AM_EXPECT(!leakReport.empty()); // Should return some string, even if stats are disabled
#endif
