...
{
    AM_TEST_CASE(EngineTestCase, core_driver, null_driver_can_be_initialized)
    {
    public:
        void Run() override
        {
            AM_EXPECT(amEngine->GetDriver() != nullptr);
            AM_EXPECT_EQ("null", amEngine->GetDriver()->GetName());
    ...
    AM_REGISTER_TEST(core_driver, null_driver_can_be_initialized);
} // namespace SparkyStudios::Audio::Amplitude::Tests
