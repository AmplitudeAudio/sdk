...
{
    AM_TEST_CASE(NodeTestCase, mixer_nodes, clamp_node_clamps_out_of_range)
    {
    public:
        void Run() override
        {
            constexpr AmUInt64 frameCount = 6;
            ...
            AM_EXPECT(std::abs((*output)[0][1] - (-1.0f)) < 1e-6f);
            AM_EXPECT(std::abs((*output)[0][2] - 0.0f) < 1e-6f);
            AM_EXPECT(std::abs((*output)[0][3] - 0.5f) < 1e-6f);
            AM_EXPECT(std::abs((*output)[0][4] - 1.0f) < 1e-6f);
            AM_EXPECT(std::abs((*output)[0][5] - 1.0f) < 1e-6f);
        }
    };

    AM_REGISTER_TEST(mixer_nodes, clamp_node_clamps_out_of_range);
} // namespace SparkyStudios::Audio::Amplitude::Tests
