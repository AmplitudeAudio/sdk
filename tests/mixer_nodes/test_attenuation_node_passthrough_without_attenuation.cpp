...
{
    AM_TEST_CASE(NodeTestCase, mixer_nodes, attenuation_node_passthrough_without_attenuation)
    {
    public:
        void Run() override
        {
            ...
            // No attenuation set on mock layer (default is nullptr)
            ...
            // With no attenuation, the node should return the input directly (passthrough)
            AM_EXPECT(output == &input);
        }
    };

    AM_REGISTER_TEST(mixer_nodes, attenuation_node_passthrough_without_attenuation);
} // namespace SparkyStudios::Audio::Amplitude::Tests
