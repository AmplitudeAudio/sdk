...
{
    AM_TEST_CASE(NodeTestCase, mixer_nodes, limiter_node_passes_quiet_signal)
    {
    public:
        void Run() override
        {
            ...
            for (AmUInt64 i = 0; i < frameCount; ++i)
                input[0][i] = 0.3f * std::sin(2.0f * AM_PI32 * 440.0f * static_cast<AmReal32>(i) / static_cast<AmReal32>(sampleRate));

            const AudioBuffer* output = processor->Process(&input);
            AM_EXPECT_NOT(output == nullptr);

            for (AmUInt64 i = 100; i < frameCount; ++i)
                AM_EXPECT(std::abs((*output)[0][i] - input[0][i]) < 0.05f);
        }
    };

    AM_REGISTER_TEST(mixer_nodes, limiter_node_passes_quiet_signal);
} // namespace SparkyStudios::Audio::Amplitude::Tests
