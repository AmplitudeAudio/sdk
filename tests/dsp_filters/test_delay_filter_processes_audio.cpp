...
{
    AM_TEST_CASE(DSPTestCase, dsp_filters, delay_filter_processes_audio)
    {
    public:
        void Run() override
        {
            auto filter = amshared(DelayFilter);
            filter->Initialize(0.3f, 0.7f, 0.0f);
            ...
            // Verify output buffer has non-zero values (delayed signal)
            AM_EXPECT(EnsureHasNonZeroOutput(outputBuffer));
        }
    };

    AM_REGISTER_TEST(dsp_filters, delay_filter_processes_audio);
} // namespace SparkyStudios::Audio::Amplitude::Tests
