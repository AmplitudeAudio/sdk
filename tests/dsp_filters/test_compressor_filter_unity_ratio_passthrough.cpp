...
{
    AM_TEST_CASE(DSPTestCase, dsp_filters, compressor_filter_unity_ratio_passthrough)
    {
    public:
        void Run() override
        {
            ...
            // Ratio 1:1 means no compression regardless of threshold
            filter->Initialize(-20.0f, 1.0f, 5.0f, 100.0f, 0.0f);
            ...
            // Ratio 1:1 → slope = 0 → no gain reduction
            AM_EXPECT(std::abs(outputRMS - inputRMS) / inputRMS < 0.05f);
        }
    };

    AM_REGISTER_TEST(dsp_filters, compressor_filter_unity_ratio_passthrough);
} // namespace SparkyStudios::Audio::Amplitude::Tests
