...
{
    AM_TEST_CASE(DSPTestCase, dsp_filters, lofi_filter_processes_audio)
    {
    public:
        void Run() override
        {
            ...
            // Verify output buffer has non-zero values (lo-fi signal)
            AM_EXPECT(EnsureHasNonZeroOutput(outputBuffer));
        }
    };

    AM_REGISTER_TEST(dsp_filters, lofi_filter_processes_audio);
} // namespace SparkyStudios::Audio::Amplitude::Tests
