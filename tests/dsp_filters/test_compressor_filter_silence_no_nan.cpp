...
{
    AM_TEST_CASE(DSPTestCase, dsp_filters, compressor_filter_silence_no_nan)
    {
    public:
        void Run() override
        {
            ...
            filter->Initialize(-20.0f, 4.0f, 5.0f, 100.0f, 6.0f, 12.0f);
            auto instance = filter->CreateInstance();

            // All-zero input (silence)
            ...
            {
                AM_EXPECT_NOT(std::isnan(output[0][i]));
                AM_EXPECT_NOT(std::isinf(output[0][i]));
            }
        }
    };

    AM_REGISTER_TEST(dsp_filters, compressor_filter_silence_no_nan);
} // namespace SparkyStudios::Audio::Amplitude::Tests
