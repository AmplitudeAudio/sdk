...
{
    AM_TEST_CASE(DSPTestCase, dsp_filters, biquad_high_pass_attenuates_low_freq)
    {
    public:
        void Run() override
        {
            ...
            filter->InitializeHighPass(1000.0f, 0.707107f);
            auto instance = filter->CreateInstance();
            AM_EXPECT_NOT(instance == nullptr);

            // Generate signal with 100 Hz (should be attenuated) and 5000 Hz (should pass)
            ...
            // Low frequency attenuated: overall RMS should drop
            AM_EXPECT(outputRMS < inputRMS * 0.8f);

            // High frequency still present: output should not be silent
    ...
    AM_REGISTER_TEST(dsp_filters, biquad_high_pass_attenuates_low_freq);
} // namespace SparkyStudios::Audio::Amplitude::Tests
