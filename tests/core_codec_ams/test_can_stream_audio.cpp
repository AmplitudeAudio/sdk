...
{
    AM_TEST_CASE(DSPTestCase, core_codec_ams, can_stream_audio)
    {
    public:
        void Run() override
        {
            ...
            AudioBuffer streamBuffer(1024, numChannels);
            AmUInt64 streamed = decoder->Stream(&streamBuffer, 0, 512, 1024);
            AM_EXPECT(streamed > 0);

            decoder->Close();
        }
    };

    AM_REGISTER_TEST(core_codec_ams, can_stream_audio);
} // namespace SparkyStudios::Audio::Amplitude::Tests
